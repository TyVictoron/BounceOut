...
class ViewController3: UIViewController, UICollisionBehaviorDelegate {
    ...
    override func viewDidLoad() {
        ...
        for var x = xOffset; x < width; x += 42 {addBlock(x, y:  40, color: UIColor.orangeColor())}
        for var x = xOffset; x < width; x += 42 {addBlock(x, y:  62, color: UIColor.blueColor())}
        for var x = xOffset; x < width; x += 42 {addBlock(x, y:  84, color: UIColor.blueColor())}
        ...
        for var x = xOffset; x < width; x += 42 {addBlock(x, y:  150, color: UIColor.yellowColor())}
    ...
    func collisionBehavior(behavior: UICollisionBehavior, beganContactForItem item1: UIDynamicItem, withItem item2: UIDynamicItem, atPoint p: CGPoint) {
        ...
        for block in bricks {
            if item1.isEqual(ball) && item2.isEqual(block) {
                if block.backgroundColor == UIColor.orangeColor() {
    ...
    func resetGame(winLossText: String) {
        ...
        let resultController = storyboard?.instantiateViewControllerWithIdentifier("MenuID") as MenuViewController
